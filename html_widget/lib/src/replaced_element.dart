...
class TextContentElement extends ReplacedElement {
  ...
  @override
  Widget toWidget(RenderContext context) => null;
}
...
class IframeContentElement extends ReplacedElement {
  ...
  Widget toWidget(RenderContext context) {
    ...
    return Container(
      width: width ?? 360,
      height: height ?? (width ?? 360) / 1.7777,
...
class RubyElement extends ReplacedElement {
  ...
  Widget toWidget(RenderContext context) {
    ...
    // ignore: todo
    // TODO(anonymous): calculate based off of parent font size.
...
ReplacedElement parseReplacedElement(dom.Element element, double width) {
  switch (element.localName) {
    ...
    case 'iframe':
      print('width ${element.attributes['width']}');

      return IframeContentElement(
        name: 'iframe',
        src: element.attributes['src'],
        width:
            double.tryParse(element.attributes['width'] ?? '$width' ?? '160'),
